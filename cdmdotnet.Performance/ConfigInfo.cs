...
namespace cdmdotnet.Performance
{
	...
	public class ConfigInfo
	{
		...
		/// </summary>
		public const string CategoryNameKey = "cdmdotnet.Performance.PerformanceCounterCategory";
		...
		/// <summary>
		/// Gets the name of the Performance Counter Category to use
		/// </summary>
		/// <remarks>
		...
		/// </summary>
		private static readonly Lazy<ConfigInfo> configInfo = new Lazy<ConfigInfo>(InitializeConfigInfo);
		...
		{
			ConfigInfo info = new ConfigInfo {ProcessId = Process.GetCurrentProcess().Id};

			string categoryName = ConfigurationManager.AppSettings[CategoryNameKey];
			...
			else
			{
				...
				info.PerformanceCategoryName = categoryName;
				...
				try
				{
					...
					{
						string message = string.Format("The performance category was set to '{0}', but this category does not exist on this machine", categoryName);
